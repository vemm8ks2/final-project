...
const CartItem = ({ item }) => {
  ...
  const handleUpdate = (num) => {
    dispatch(updateCartItem({ data: { quantity: item.quantity + num }, cartItemId: item?.id }));
  };
    ...
    <div className="p-5 shadow-lg border rounded-md">
      ...
      <div className="lg:flex items-center lg:space-x-10 pt-4">
        <div className="flex items-center space-x-2">
          ...
          <IconButton onClick={() => handleUpdate(1)} sx={{ color: 'rgb(145 85 253)' }}>
            <AddCircleOutlineIcon />
          </IconButton>
        </div>
        <div>
          <Button onClick={handleRemove} sx={{ color: 'rgb(145 85 253)' }}>
            remove
          </Button>
        </div>
      </div>
